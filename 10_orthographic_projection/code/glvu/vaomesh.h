...
struct GLMesh
{
    ...
    std::array<R, 16> matMVP(const int *vp, bool isPerspective, bool offsetDepth = false, bool colmajor = false) const {
        ...
        Mat4 proj;
        if(isPerceptive) 
            proj = perspective(45.f, vp[2] / float(vp[3]), 0.1f, 100.0f);
		else 
            proj = orthographic(100.f, vp[2] / float(vp[3]), 0.1f, 100.0f);
    ...
    void draw(const int *vp,bool isPerspective=true)
    {
        glEnable(GL_BLEND);
        ...
        prog.setUniform("MVP", matMVP(vp,isPerceptive).data());
        ...
        prog.setUniform("MVP", matMVP(vp, isPerceptive,true).data());
        if (showWireframe()) {
